...
const AT_Cmd_t AT_Cmd_Arr[]={
  ...
  {"IR",      AT_Cmd_IR,      "execute command IR:<Address>,<code1>,<code2>,<code3>"},
...
/*****************************************************
  AT+IR:<Address>,<code1>,<code2>,<code3>
  AT+IR:,,,
*********************************************************/
void AT_Cmd_IR(uint8 start_point, uint8* msg){
  ...
  buff.cmdIR=SEND_IR_CMD;
  buff.data[0]=0xA1;
  buff.data[1]=0xF1;
  for(i=1,j=2;i<4;i++,j++){
    buff.data[j]=AT_ChartoInt8(&cmdUnitArr[i]);
  }
  uint16 nwk =AT_ChartoInt16(&cmdUnitArr[0]);
  //uint8 nwk = 0xffff;
  //HalUARTWrite(HAL_UART_PORT_1,(uint8*)&buff.data, sizeof(buff.data));
